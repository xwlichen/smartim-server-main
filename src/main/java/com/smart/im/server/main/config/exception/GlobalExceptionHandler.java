...
public class GlobalExceptionHandler {
    ...
    public Map<String,Object> errorHandler(Exception e) {
        ...
        map.put(Constants.STATUS_STR,Constants.OTHER_ERROR);
        //如果是路径映射错误,直接返回给前端
        if (e instanceof NoHandlerFoundException) {
            map.put(Constants.MESSAGE_STR,String.format("请求地址{%s}错误,请检查后再次尝试!", SpringContextHolder.getRequest().getRequestURI()));
        } else {
            map.put(Constants.MESSAGE_STR,"服务器异常!");
        }
